...
//    private int time2 = 30 * 60*1000; //12小时
//    private int time2 = 10 * 60 * 1000; //12小时

    private static int index = 0;

    private volatile static boolean flag = false; //默认式false
...
//                            Toast.makeText(getApplicationContext(), "position " + position + " title:" + title + " is clicked.", Toast.LENGTH_SHORT).show();
                            if (position == 0) {
                                index =0;
                                showToastByRunnable(MainService.this, "正在启动");
                                DataCleanManager.clearAllCache(MainService.this);
                                showToastByRunnable(MainService.this, "正在清理缓存");
...
//                                WatchDog.stopTimer();
                                ...
                                try {
                                    showToastByRunnable(MainService.this, "正在停止");
...
//        if (timerTask2==null) {
        timerTask2 = new TimerTask() {
            @Override
            public void run() {
                Log.e(Constant.TAG,"shaomiaodingshiqidinagshiqi");
                startTimer();
                flag = !flag;
            }
        };
